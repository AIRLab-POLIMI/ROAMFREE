...
int main(int argc, char *argv[]) {
	...
	ParameterWrapper_Ptr ba_par = StochasticProcessFactory::addEucl3DRandomWalk(
			f, "IMUintegralDeltaP_Ba", accBias0, randomWalkNoiseVar, 1.0); // 1.0 -> keep one sample every 1.0s
	...
	ParameterWrapper_Ptr bw_par =
			StochasticProcessFactory::addEucl3DGaussMarkovPlusRandomConstant(f,
					"IMUintegralDeltaP_Bw", gyroBias0_RC, gyroBias0_GM,
					gaussMarkovBeta, gaussMarkovNoiseVar, 1.0); // 1.0 -> keep one sample every 1.0s
...
#   include "../generated/Otto_x0.cppready"
	...
	f->addPriorOnTimeVaryingParameter(Euclidean3DPrior,
			"IMUintegralDeltaP_Bw_GM", f->getOldestPose()->getTimestamp(),
