...
namespace g2o {
  ...
  /**
   * An extension to the previous method to allow B being a Matrix. It simply iterates the above method
   * for each column of B computing the cholesky decomposition of A just one time
   */
  int cs_cholsolsymb_matrixB(const cs *A, double *B, int Bn, const css* S, double* x, int* work)
  {
    csn *N ;
    int n, ok ;
    if (!CS_CSC (A) || !B || ! S || !x) {
    ...
    if (ok)
    {
      /* for each column of B <- which is column-major! */
      for (int k = 0; k < Bn; k++) {
        double *b = B+k*A->n; /* select the current working column of B */
