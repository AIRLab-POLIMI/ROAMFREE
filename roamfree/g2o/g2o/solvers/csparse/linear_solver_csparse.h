  ...
  public:
    ...
    bool solve(const SparseBlockMatrix<MatrixType>& A, double* x, double* b)
    {
      ...
      if (! ok) {
        ...
        writeCs2Octave("debug/H.txt", _ccsA, true);
        return false;
      }

      // debug, always save hessian
      // writeCs2Octave("debug/H.txt", _ccsA, true);
    ...
    bool invAmultB(const SparseBlockMatrix<MatrixType>& A, MatrixXd &B)
    {
      fillCSparse(A, _symbolicDecomposition);
      // perform symbolic cholesky once
      if (_symbolicDecomposition == 0) {
      ...
      int ok = cs_cholsolsymb_matrixB(_ccsA, B.data(), B.cols(), _symbolicDecomposition, _csWorkspace, _csIntWorkspace);
      if (! ok) {
        std::cerr << "Cholesky failure, writing debug.txt (Hessian loadable by Octave)" << std::endl;
        writeCs2Octave("debug/H.txt", _ccsA, true);
        return false;
      ...
      if (globalStats){
        globalStats->timeinvAmultB = get_time() - t;
        //globalStats->choleskyNNZ = _symbolicDecomposition->lnz;
      }
