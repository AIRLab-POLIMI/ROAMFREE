...
OptimizableGraph::Vertex::Vertex() :
    HyperGraph::Vertex(), _graph(0), _userData(0), _tempIndex(-1), _fixed(
        false), _marginalized(false), _colInHessian(-1) {
...
double * OptimizableGraph::Vertex::accessEstimateData() {
  return NULL;
}
...
OptimizableGraph::Edge::Edge() :
    HyperGraph::Edge(), _dimension(-1), _level(0), _userData(0), _robustKernel(false), _huberWidth(
        1.) {
}

void OptimizableGraph::Edge::afterVertexUpdate() { }

bool OptimizableGraph::Edge::setMeasurementData(const double *) {
  return false;
}

double * OptimizableGraph::Edge::accessMeasurementData() {
  return NULL;
}
...
bool OptimizableGraph::addEdge(OptimizableGraph::Edge* e) {
  OptimizableGraph::Edge* eresult =
      dynamic_cast<OptimizableGraph::Edge*>(HyperGraph::addEdge(e));
  if (!eresult)
    return false;
...
bool OptimizableGraph::save(ostream& os, int level) const {
  ...
  for (set<Vertex*, VertexIDCompare>::const_iterator it =
      verticesToSave.begin(); it != verticesToSave.end(); ++it) {
...
void OptimizableGraph::addGraph(OptimizableGraph* g) {
  ...
  for (HyperGraph::EdgeSet::iterator it = g->edges().begin();
    ...
    for (vector<HyperGraph::Vertex*>::const_iterator it = e->vertices().begin();
        it != e->vertices().end(); ++it) {
      OptimizableGraph::Vertex* v = (OptimizableGraph::Vertex*) vertex(
          (*it)->id());
      assert(v);
      en->vertices()[cnt++] = v;
    }
...
void OptimizableGraph::setRenamedTypesFromString(const std::string& types) {
  ...
  for (std::map<std::string, std::string>::const_iterator it =
      _renamedTypesLookup.begin(); it != _renamedTypesLookup.end(); ++it) {
    cerr << "#\t" << it->first << " -> " << it->second << endl;
  }
}

bool OptimizableGraph::isSolverSuitable(const SolverProperty& solverProperty,
  ...
  if (vertDims.size() == 2) {
    ...
    suitableSolver = solverProperty.poseDim == -1
        && !solverProperty.requiresMarginalize;
  }
