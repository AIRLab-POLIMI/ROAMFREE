...
#include "PriorEdges/Eucl2DRandomWalkProcessEdge.h"

namespace ROAMestimation {

Eucl2DRandomWalkProcessEdge::Eucl2DRandomWalkProcessEdge() {
  _measurement.resize(2);
  _measurement.setZero(); // by default the prior on the derivative is zero
}

void Eucl2DRandomWalkProcessEdge::computeError() {
...
void Eucl2DRandomWalkProcessEdge::linearizeOplus() {
  // do nothing, _jacobianOplusXj and _jacobianOplusXi are constant
}

std::string Eucl2DRandomWalkProcessEdge::writeDebugInfo() const {
  std::stringstream s;
  ...
  s << "Eucl2DRandomWalkProcessEdge(" << x0->id() << "," << x1->id() << ")";
