...
public:
	...
	virtual std::list<ParameterWrapper_Ptr> getCrossCovariance()=0;
	virtual void addCrossCovarianceParameter(ParameterWrapper_Ptr paramCross)=0;
	...
	virtual void setRandomWalkNoiseCov(const Eigen::MatrixXd &cov) = 0; //!< sets the *continuous time* noise covariance matrix for the Random-Walk process (in <unit>/Hz)
	virtual void setGaussMarkovNoiseCov(const Eigen::MatrixXd &cov) = 0; //!< sets the *continuous time* noise covariance matrix for the Gauss-Markov process (in <unit>/Hz)
