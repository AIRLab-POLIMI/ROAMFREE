...
public:
  ...
  bool write(std::ostream &s) const {
    s << (_category.length() > 0 ? _category : "unknown") << " " << _fixed
        << " " << _displayEstimate << " ";
    for (unsigned int i = 0; i < _estimate.rows(); i++) {
      s << std::setprecision(8) << _estimate(i) << " ";
    }
  ...
  bool read(std::istream &s) {
    s >> _category >> _fixed >> _displayEstimate;

    // shoud be safe since estimate is resized depending on the vertex type in constructor
    for (unsigned int i = 0; i < _estimate.rows(); i++) {
      s >> _estimate(i);
    }
